...
public class CartContentsIT extends BaseIT {
    ...
    public void setup() throws Exception{
        ...
        client.addFoodToCart("test@test", foodId1, 1);
        client.addFoodToCart("test@test", foodId2, 2);
    }
    ...
    public void success() throws Exception {
        List<FoodOrderItem> contents = client.cartContents("test@test");

        Assert.assertEquals("T28_Restaurant1", contents.get(0).getFoodId().getRestaurantId());
        Assert.assertEquals("Barato", contents.get(0).getFoodId().getMenuId());
        Assert.assertEquals(1, contents.get(0).getFoodQuantity());

        Assert.assertEquals("T28_Restaurant2", contents.get(1).getFoodId().getRestaurantId());
        Assert.assertEquals("Barato2", contents.get(1).getFoodId().getMenuId());
        Assert.assertEquals(2, contents.get(1).getFoodQuantity());
    }
    ...
    public void nonActivatedUserId() throws Exception {
        Assert.assertTrue(client.cartContents("wtv@wtv.org").isEmpty());
    }
}
