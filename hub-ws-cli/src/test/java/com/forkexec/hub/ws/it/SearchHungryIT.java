...
public class SearchHungryIT extends BaseIT {
    ...
    public void success() throws Exception {
        List<Food> food = client.searchHungry("Nuggets");
        Assert.assertEquals(2, food.size());

        Assert.assertEquals("Barato2", food.get(0).getId().getMenuId());
        Assert.assertEquals("T28_Restaurant2", food.get(0).getId().getRestaurantId());
        Assert.assertEquals("Barato", food.get(1).getId().getMenuId());
        Assert.assertEquals("T28_Restaurant1", food.get(1).getId().getRestaurantId());
    }
